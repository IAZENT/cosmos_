 ...
 * to forward. Used from Next middleware.ts to gate /admin routes.
 */
export async function updateSession(request: NextRequest) {
  ...
  if (!url || !anonKey) {
    // No Supabase configured  let the request through; /admin pages will
  ...
  // Redirect disallowed users hitting /admin/* (except /admin/login) to login.
  ...
  // When an authenticated+allowed user visits /admin/login, send them home.
